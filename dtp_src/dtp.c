...
int main(int argc, char *argv[]) {
    ...
    if (fgetc(fp) != head[0]) {
        for (int i = 0; i != 4;) {
            ...
            if (cur_pos == filesize) {
                die("not found the head\n");
            }
        }
        cur_pos = ftell(fp) - 4;
        fseek(fp, 0, SEEK_SET);
        sprintf(filename, "%s-%d", file_, new_dtb_count++);
        FILE *new_kernel = fopen(filename, "wb");
        for (uint32_t i = 0; i < cur_pos; i++) {
            buff_c = fgetc(fp);
            fputc(buff_c, new_kernel);
        }
        fclose(new_kernel);
    }
    fseek(fp, cur_pos, SEEK_SET);
    ...
    while (1) {
        ...
        if (memcmp(&buff, &head, sizeof(buff))) {
            break;
        }
